...
#from pynndescent import initialise_search, initialized_nnd_search
...
def _matplotlib_points(
    ...
    if show_legend and legend_elements is not None:
        #ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1,1.005), frameon=False)
        if legend_pos=='bottomLeft':
            ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(0,0), ncol=3, frameon=False)
        elif legend_pos=='rightOutside':
            ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1.05,1), ncol=3, frameon=False)
...
def diagnostic(
    ...
    if point_size is None:
        point_size = 400.0 / np.sqrt(points.shape[0])
